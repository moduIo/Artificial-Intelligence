...
def move():
    ...
    tiles = list(map(int, tiles_line))  # read the original state
    if len(tiles) == 0:
    ...
    if len(tiles) == 9:
        mod = 3
        goal = list(map(int, GOAL3.split(",")))    # the supposed final state
    elif len(tiles) == 16:
        mod = 4
        goal = list(map(int, GOAL4.split(",")))    # the supposed final state
    else:
